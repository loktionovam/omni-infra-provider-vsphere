...
#
# Generated on 2026-03-06T12:47:31Z by kres 1dd7316.
...
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni-infra-provider-vsphere/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni-infra-provider-vsphere/go/pkg go install github.com/golangci/golangci-lint/v2/cmd/golangci-lint@${GOLANGCILINT_VERSION} \
	&& mv /go/bin/golangci-lint /bin/golangci-lint
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni-infra-provider-vsphere/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni-infra-provider-vsphere/go/pkg go install golang.org/x/vuln/cmd/govulncheck@latest \
...
COPY go.sum go.sum
RUN cd .
RUN --mount=type=cache,target=/go/pkg,id=omni-infra-provider-vsphere/go/pkg go mod download
RUN --mount=type=cache,target=/go/pkg,id=omni-infra-provider-vsphere/go/pkg go mod verify
COPY ./api ./api
COPY ./cmd ./cmd
...
WORKDIR /src
ARG TESTPKGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni-infra-provider-vsphere/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni-infra-provider-vsphere/go/pkg --mount=type=cache,target=/tmp,id=omni-infra-provider-vsphere/tmp CGO_ENABLED=1 go test -race ${TESTPKGS}
...
FROM scratch AS omni-infra-provider-vsphere-all
COPY --from=omni-infra-provider-vsphere-linux-amd64 / /

FROM scratch AS image-omni-infra-provider-vsphere
ARG TARGETARCH
